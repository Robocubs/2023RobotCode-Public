...
public class FalconSwerveModule extends Subsystem {
    ...
    private final double kDrivePositionCoefficient =
            Math.PI * Constants.kDriveWheelDiameter * Constants.kDriveReduction / 2048.0;
    ...
    public FalconSwerveModule(int driveId, int steeringId, CANCoder cancoder, Rotation2d encoderZero) {
    ...
    public void configureTalons() throws RuntimeException {
        ...
        TalonUtil.checkErrorWithThrow(
                mDriveMotor.configSupplyCurrentLimit(
                        new SupplyCurrentLimitConfiguration(false, 100.0, 120.0, 0.0), Constants.kLongCANTimeoutMs),
                "Failed to set supply current limit");
        TalonUtil.checkErrorWithThrow(
                mDriveMotor.configStatorCurrentLimit(
                        new StatorCurrentLimitConfiguration(true, 100.0, 120.0, 0.0), Constants.kLongCANTimeoutMs),
        ...
        mDriveMotor.setInverted(TalonFXInvertType.CounterClockwise);
        ...
        // PID
        // TODO: Remove dependency on Constants
        TalonUtil.checkErrorWithThrow(
                mSteeringMotor.config_kP(0, Constants.kMk4AziKp, Constants.kLongCANTimeoutMs), "Failed to set kP");
        TalonUtil.checkErrorWithThrow(
                mSteeringMotor.config_kI(0, Constants.kMk4AziKi, Constants.kLongCANTimeoutMs), "Failed to set kI");
        TalonUtil.checkErrorWithThrow(
                mSteeringMotor.config_kD(0, Constants.kMk4AziKd, Constants.kLongCANTimeoutMs), "Failed to set kD");
        TalonUtil.checkErrorWithThrow(
    ...
    public void rezeroSteeringMotor() {
        mTalonOffset = Rotation2d.fromRadians(mSteeringMotor.getSelectedSensorPosition() * kSteerPositionCoefficient)
                .rotateBy(getAdjustedCanCoderAngle().unaryMinus());
    }
    ...
    public Rotation2d getAdjustedCanCoderAngle() {
        return getCanCoderAngle().rotateBy(mEncoderZero.unaryMinus());
    }
    ...
    public double getSteerClosedLoopError() {
        return mSteeringMotor.getClosedLoopError() * kSteerPositionCoefficient;
    }
    ...
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(getDriveDistance(), getSteerAngle());
    }
    ...
    public void setWithVelocityShortestPath(double driveVelocity, Rotation2d steerAngle) {
        final boolean flip = setSteerAngleShortestPath(steerAngle);
        mDriveMotor.set(
                TalonFXControlMode.Velocity, (flip ? -driveVelocity : driveVelocity) / kDriveVelocityCoefficient);
    }
    ...
    private void setSteerAngleUnclamped(double steerAngleRadians) {
        mSteeringMotor.set(
                ...
                (steerAngleRadians + mTalonOffset.getRadians()) / kSteerPositionCoefficient);
    }
